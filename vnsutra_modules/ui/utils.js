...
function getRadioOptions(options, name, large) {
    let text = '';
    ...
    for (let i=0;i<options.length;i++) {
        ...
        <li>
            ...
            <label for="${name}-option-${i}" class="inline-flex items-center justify-between w-full p-3 px-4 border-2 rounded-full cursor-pointer" style="color: ${configuration.colors.text}; border-color: ${configuration.colors.text};"> 
                <div class="block">
                    <div class="w-full ${ large ? "text-lg" : "text-sm" }">${option}</div>
                </div>
            </label>
        </li>
        `;
...
class LongTextRadio {
    constructor({ label, id, placeholder = "", large = false, answer = null, options = [], onchange = (e, input) => {}, disabled = false }) {
        ...
        <div class="relative flex w-full max-w-full flex-col">
            <ul class="flex flex-col w-full gap-3">
                ${getRadioOptions(options, id, large)}
            </ul>
        </div>
        `;
                ...
                elem.onclick = (e) => {
                    ...
                    if(answer) {
                        if(radio.value === answer) {
                            ...
                            let elem2 = document.getElementById(id).querySelector(`input[value='${answer}']`).nextElementSibling;
                            elem2.style.borderColor = "lightgreen";
                            elem2.style.color = "lightgreen";
                        }

                        onchange({...e, correct: radio.value === answer}, radio);
                    }
